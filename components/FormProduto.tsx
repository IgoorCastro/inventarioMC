...
import { Box, HStack, Input, VStack, Text, FlatList } from "native-base";
...
export default function ({ onSubmit, isModalOpen, quantVendaProp }: FormProdutoProps) {
    ...
    const handleInputProdChange = (value: string, onChange: (value: string) => void) => {
        onChange(value);
        const filtered = listProdutos.filter((produto) => produto.nome.toLowerCase().includes(value.toLowerCase()));
    ...
    const procurarProdutoById = (id: string) => {
        return listProdutos.find((item) => item.id.toLowerCase() === id.toLowerCase());
    };

    const procurarProdutoByNome = (nome: string) => {
        return listProdutos.find((item) => item.nome.toLowerCase().includes(nome.toLowerCase()));
    };

    const onChangeQnt = (value: number) => {
        let nValue = value > 0 ? value : 1;
    ...
    return (
        ...
        >
            ...
            >
                    ...
                    <Controller
                        ...
                        render={({ field: { onChange, onBlur, value = "" } }) => (
                            <Input
                                ...
                                variant="rounded"
                                onChangeText={(text) => handleInputIdChange(text, onChange)}
                ...
                <input ref={buttonRef} type="submit" value="Confirmar Produto" style={{ padding: 7 }} />
            </VStack>
        </form>
    )
}